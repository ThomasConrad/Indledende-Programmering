...
public class Racetrack {
	...
	public static void main(String[] args) {
		...
		// Skab arrays til at holde spillere og sejrherrer
		...
		for (int i = 0; i < playerCount; i++) {
			players.add(new Player(start[0], start[1],
					new Color(randGen.nextInt(256), randGen.nextInt(256), randGen.nextInt(256)), (i + 1))); // spillerne
																											// bliver
																											// givet en
																											// tilfældig
																											// farve
		...
		while (players.size() > 0) {
			for (int i = 0; i < players.size(); i++) {
				...
				// Input tages direkte fra tastaturet!
				...
				int d = Character.getNumericValue(StdDraw.nextKeyTyped());
				...
				/*
				 * if (Segment.collision(last,goal)) { //Tjek om man krydser med uret ved at
				 * tage determinanten af vektorerne dannet af linjesegmenterne. //Hvis
				 * determinanten er positiv krydses der med uret, under antagelse at målet som
				 * vekter pejer væk fra centrum. if
				 * ((pc.x-pc.prevx)*(goal.B[1]-goal.A[1])-(pc.y-pc.prevy)*(goal.B[0]-goal.A[0])
				 * > 0) { laps += 1; } else { laps -= 1; } System.out.println("LAPS: " + laps);
				 * }
				 */

				// Revideret version af ovenstående kode, tilføjer eventuelle vindere samt hvor
				// hurtigt de gjorde det
				if (Segment.collision(last, goal)) {
					// Tjek om man krydser med uret ved at tage determinanten af vektorerne dannet
					// af linjesegmenterne.
					// Hvis determinanten er positiv krydses der med uret, under antagelse at målet
					// som vekter pejer væk fra centrum.
	...
	public static class Player {
		...
		// Flytter efter reglerne for vektor-rally hvor dx og dy ifølgende reglerne for
		// spillet skal være lig -1, 0, 1.
	...
	public static class Segment {
		...
		// Alt er privat da alle variabler for den generelle form afhænger af punkterne
		// A og B
		...
		Segment(double a1, double a2, double b1, double b2) {
			...
			// Konstanterne M og K, svarende til hældning og skæring med y-aksen, udregnes
			// ud fra koordinaterne af punkterne A og B.
			if (a1 == b1) {
				this.vertical = true;
			} else {
				this.M = (b2 - a2) / (b1 - a1);
		...
		public static boolean collision(Segment l1, Segment l2) {
			...
			// Tjekker om skæringspunktet ligger inden for intervallet af x- og
			// y-koordinater for både l1 og l2.
		...
		/*
		 * Segment(double M, double K, double a1, double b1) { this.M = M; this.K = K;
		 * this.A[0] = a1; this.B[0] = b1; this.A[1] = M*a1+K; this.B[1] = M*b1+K; if
		 * (a1 == b1) { this.vertical = true; } }
		 */
